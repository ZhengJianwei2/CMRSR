...
class TrainSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h, w, _ = LR.shape
        LR_sr = np.array(Image.fromarray(LR).resize((w*4, h*4), Image.BICUBIC))
...
class Urban100(Dataset):
    def __init__(self, args):
        self.input_list = sorted(glob.glob('/data/liuyu/urban100/image_SRF_4/*.png'))
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
...
class Manga109(Dataset):
    def __init__(self, args):
        self.input_list = sorted(glob.glob('/data/liuyu/manga109/*.png'))
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
...
class Sun80(Dataset):
    def __init__(self, args):
        self.input_list = sorted(glob.glob('/data/liuyu/Sun80/Sun_Hays_SR_groundtruth/*.jpg'))
        self.ref_folders = '/data/liuyu/Sun80/Sun_Hays_SR_scenematches'
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
...
class DIV2K(Dataset):
    def __init__(self, args):
        self.input_list = sorted([os.path.join(args.data_root, args.dataset, 'DIV2K_train_HR', name) for name in
                                  os.listdir(os.path.join(args.data_root, args.dataset, 'DIV2K_train_HR'))])
        self.scale = args.sr_scale
        self.data_augmentation = args.data_augmentation
        self.transform = transforms.ToTensor()
    ...
    def __getitem__(self, idx):
        ...
        h, w, _ = HR.shape
        Ref_down = np.array(Image.fromarray(HR).resize((w // self.scale, h // self.scale), Image.BICUBIC))
...
class DIV2K_TestSet(Dataset):
    def __init__(self, args):
        ref_level = args.ref_level
        # ref_level = 5
        self.input_list = sorted(glob.glob(os.path.join(args.data_root, args.dataset, 'DIV2K_valid_HR', '*.png')))
        self.scale = args.sr_scale
        self.transform = transforms.ToTensor()
