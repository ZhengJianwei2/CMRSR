...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.phase == 'train':
            if args.dataset=='CUFED':
                ...
                trainset_=getattr(importlib.import_module('dataloader.dataset'), args.dataset, None)
        ...
        if args.rank <= 0:
            self.logger.info('----- generator parameters: %f -----' % (sum(param.numel() for param in self.net.parameters()) / (10**6)))

        ## init loss and optimizer
        if args.phase == 'train':
            if args.rank <= 0:
                self.logger.info('init criterion and optimizer...')
            ...
            if args.loss_l1:
                ...
                if args.rank <= 0:
                    self.logger.info('  using l1 loss...')
            ...
            if args.loss_adv:
                ...
                if args.rank <= 0:
                    self.logger.info('  using adv loss...')
            ...
            if args.loss_perceptual:
                ...
                if args.rank <= 0:
                    self.logger.info('  using perceptual loss...')
    ...
    def set_learning_rate(self, optimizer, epoch):
        ...
        if self.args.rank <= 0:
            self.logger.info('current_lr: %f' % (current_lr))
    ...
    def train(self):
        if self.args.rank <= 0:
            self.logger.info('training on  ...' + self.args.dataset)
            self.logger.info('%d training samples' % (self.train_dataset.__len__()))
            self.logger.info('the init lr: %f'%(self.args.lr))
        steps = 0
        self.net.train()
        ...
        for i in range(self.args.start_iter, self.args.max_iter):
            self.scheduler.step()
            self.logger.info('current_lr: %f' % (self.optimizer_G.param_groups[0]['lr']))
            t0 = time.time()
            ...
            if i % self.args.save_epoch_freq == 0:
                if self.args.rank <= 0:
                    ...
                    self.logger.info('Saving state, epoch: %d iter:%d' % (i, 0))
            ...
            if not self.args.loss_adv:
                if i > -1:
                    ...
                    self.logger.info('psnr:%.06f   ssim:%.06f ' % (psnr, ssim))
                    
                    if psnr > self.best_psnr:
                        self.best_psnr = psnr
                        if self.args.rank <= 0:
                            self.logger.debug('best_psnr:%.06f ' % (self.best_psnr))
                            self.logger.debug('Saving state, epoch: %d iter:%d' % (i, 0))
        ...
        if self.args.rank <= 0:
            ...
            self.logger.info('The training stage on %s is over!!!' % (self.args.dataset))
    ...
    def test(self):
        ...
        self.logger.info('%d testing samples' % (self.test_dataset.__len__()))
        num = 0
        ...
        with torch.no_grad():
            for batch, batch_samples in enumerate(self.test_dataloader):
                ...
                PSNR.append(psnr)
                SSIM.append(ssim)
                self.logger.debug('psnr: %.6f    ssim: %.6f' % (psnr, ssim))
                ...
                self.logger.info('saving %d_th image: %s' % (num, image_name))
                num += 1

        PSNR = np.mean(PSNR)
        SSIM = np.mean(SSIM)
        self.logger.debug('--------- average PSNR: %.06f,  SSIM: %.06f' % (PSNR, SSIM))

    def evaluate(self):
        scale = self.args.sr_scale
        self.net.eval()
        self.logger.debug('start testing...')
        self.logger.debug('%d testing samples' % (self.test_dataset.__len__()))
