...
class FeatureMatching(nn.Module):
    def __init__(self, ksize=3, k_vsize=1,  scale=2, stride=1, in_channel =3, out_channel =64, conv=default_conv):
        ...
        self.stride = stride  
        self.scale = scale
        match0 =  BasicBlock(conv, 256, 16, 1,stride=1,bias=True,bn=False,act=nn.LeakyReLU(0.2, inplace=True))
        
        # state_dict = torch.load('/data/xjs/wmt/pretrained/MASA-SR/vgg19-dcbb9e9d.pth')
...
class AlignedAttention(nn.Module):
    ...
    def forward(self, lr, ref, index_map, value):
        ...
        warpped_features = F.fold(warpped_value, output_size=(shape_out[2]*4, shape_out[3]*4), kernel_size=(kernel,kernel), padding=0, stride=self.scale) 
        if self.align:
          ...
          warpped_ref = F.fold(warpped_ref, output_size=(shape_out[2]*4, shape_out[3]*4), kernel_size=(kernel,kernel), padding=0, stride=self.scale)         
        #   print("warpped_features: ",warpped_features.size())
        #   print("lr: ",lr.size())
        #   print("warpped_ref: ",warpped_ref.size())
